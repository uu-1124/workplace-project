...
def append_to_file():
    """Append regenerate endpoints to department_workspaces.py"""
...
async def regenerate_sales_item(
    ...
    with db_conn() as conn:
        ...
        actor_id = actor_member_id(conn, team_id, payload.regenerate_by_member_id, auth_member, "regenerate_by_member_id")
        ...
        member = {"id": member_row[0], "display_name": member_row[1], "title": member_row[2]}
        ...
        if not original_item:
            raise HTTPException(status_code=404, detail=f"Sales {item_type} with id {item_id} not found")
        ...
        command = {
            ...
            "context": {"sales_brief": sales_brief, "existing_pipeline": pipeline}
        }
        ...
        try:
            ...
            if item_type == "lead" and draft.get("leads"):
                new_item = draft["leads"][0]
                new_item["id"] = item_id  # Keep same ID
                updated_leads = [new_item if lead.get("id") == item_id else lead for lead in pipeline.get("leads", [])]
                pipeline["leads"] = updated_leads
            elif item_type == "playbook" and draft.get("playbooks"):
                new_item = draft["playbooks"][0]
                new_item["id"] = item_id
                updated_playbooks = [new_item if pb.get("id") == item_id else pb for pb in pipeline.get("playbooks", [])]
                pipeline["playbooks"] = updated_playbooks
            elif item_type == "follow_up" and draft.get("follow_ups"):
                new_item = draft["follow_ups"][0]
                new_item["id"] = item_id
                updated_follow_ups = [new_item if fu.get("id") == item_id else fu for fu in pipeline.get("follow_ups", [])]
                pipeline["follow_ups"] = updated_follow_ups

            if not new_item:
                raise HTTPException(status_code=500, detail=f"Failed to generate new {item_type}")
            ...
            raise HTTPException(status_code=500, detail=f"Failed to regenerate {item_type}: {str(e)}")
...
async def regenerate_all_sales_items(
    ...
    with db_conn() as conn:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Failed to regenerate sales items: {str(e)}")
'''
    ...
    try:
        ...
        # Backup first
        backup_cmd = "cp /opt/workplace-ai-platform/api/app/routers/department_workspaces.py /opt/workplace-ai-platform/api/app/routers/department_workspaces.py.backup.$(date +%Y%m%d_%H%M%S)"
        stdin, stdout, stderr = client.exec_command(backup_cmd)
        stdout.read()

        # Append to file
        escaped = additions.replace("'", "'\\''")
        cmd = f"printf '%s' '{escaped}' >> /opt/workplace-ai-platform/api/app/routers/department_workspaces.py"

        stdin, stdout, stderr = client.exec_command(cmd)
        exit_code = stdout.channel.recv_exit_status()

        if exit_code == 0:
            print("Successfully appended regenerate endpoints to department_workspaces.py")
        else:
