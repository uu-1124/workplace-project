...
- Root `8c60fa1` 首次接入隔离式 PR gate；`release-readiness-gate #19`（run `29253054229`）attempt 1 在任何仓库 step 开始前因 GitHub Action 下载服务连续 `Service Unavailable` 失败，attempt 2 仅重跑失败任务后双平台成功。该基础设施失败保留，没有被冒充首次绿灯。
- `release-pr-trust` 已配置唯一 reviewer、`prevent_self_review=false`、`can_admins_bypass=false` 和 `main` 分支策略；Actions 只允许 GitHub-owned Action、强制完整 SHA pin，默认 `GITHUB_TOKEN` 为只读且不能批准 PR；`PR_RELEASE_ENVIRONMENT_CONFIGURED=true` 只在这些配置读取核验后激活。
- 文档 PR `#1`（head `e7ddcf7`，run `29254289184`）中 policy 与最终 required check 成功，broker、Linux、Windows 全部 skipped，artifact 和 deployment 均为 0。该 PR 关闭且未合并。
- 发布 PR `#2` 的首个 head `0415ebd`（run `29254782941`）在审批前无 artifact，审批后加密 broker 成功；Linux 因缓存依赖文件位于 `GITHUB_WORKSPACE` 之外而确定性失败，Windows skipped。没有重跑该 SHA；Root `f0f9d95` 将无凭证测试目录移入其自身 workspace，并新增回归防止倒退。
- 修复后的 `main` push `release-readiness-gate #20`（run `29255296970`，attempt 1）双平台成功。发布 PR `#2` 更新为 head `fdc8aa3` 后重新等待并取得 Environment 审批；`PR release readiness #3`（run `29255332408`，attempt 1）完整成功。
- broker job `86834070037` 日志未匹配私钥、bundle key 或安装 token；构件 `8280997253` 只含 `encrypted-release-bundle.mjs`、`verify-assembled-release.mjs` 和 `release-source.tar.gz.enc`。密文魔数为 `SRBNDL01`、不是 gzip，digest 为 `sha256:ca811fd86a840006c0c3d443e859e2e373c16735c74c810c4b8402a0f52830ca`。
- Linux job `86834409714` 为 `319 passed, 2 skipped`，并通过 PostgreSQL 16 从零迁移、deployment smoke、当前与固定旧 Desktop UI；Windows job `86836283398` 通过解包恢复、安装、业务恢复、升级、数据保留、故障检测、回滚和卸载。最终 `Required PR gate` job `86837600562` 成功。
- `main` 已要求 PR、最新 base 和 GitHub Actions app id `15368` 的 `Required PR gate`，对管理员生效，required approvals 为 0，禁止 force push 和删除。直接更新探针返回 422 且主干 SHA 未变化。
- 攻击 PR `#3` 让候选 workflow 的同名假 `Required PR gate` 在 base-controlled 失败之后才显示成功；真实 policy 与 required check 失败，PR 仍为 `mergeable_state=blocked`。这证明当前保护不会被同名晚到绿灯绕过。探针 PR 与分支均已删除。

以上证据关闭 R0 的 PR 信任边界；它不扩大为真实供应商模型、正式签名、高可用或客户运营闭环已经完成。
